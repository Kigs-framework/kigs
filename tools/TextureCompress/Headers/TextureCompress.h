...
namespace Kigs
{
	...
	protected:
		...
		int	checkIfNeedsAlpha(SP<Pict::TinyImage>);
		int	checkBestMatchingAlpha(SP<Pict::TinyImage> toCheck);

		bool	exportDDS(SP<Pict::TinyImage>, const std::string& outfile, int quality, int perceptual);
		...
		unsigned char* CopyToPot(SP<Pict::TinyImage>, int sizex, int sizey);
