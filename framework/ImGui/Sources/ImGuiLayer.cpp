	...
	namespace Gui
	{
		...
		void ImGuiLayer::SetStyleLightGreen()
		{
			...
			*style = ImGuiStyle();
			...
			colors[ImGuiCol_ModalWindowDimBg] = ImVec4(0.86f, 0.86f, 0.86f, 0.0f); // Disabled
		...
		void ImGuiLayer::InitModifiable()
		{
			...
			if (IsInit())
			{
				...
				SmartPointer<Pict::TinyImage>	img = Pict::TinyImage::CreateImage(mPixelData, mPixelDataWidth, mPixelDataHeight, Pict::TinyImage::RGBA_32_8888);
		...
		{
			param.sorted.push_back({ this, Maths::Hit{} });
			return;

			std::transform(param.toSort.begin(), param.toSort.end(), std::back_inserter(param.sorted), [](CoreModifiable* cm) -> std::pair<CoreModifiable*, Maths::Hit>
				{
					return { cm, Maths::Hit{} };
				});
		}
