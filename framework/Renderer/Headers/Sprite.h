...
#define KIGS_SPRITE_DEFAULT_DISPLACEMENT (0.002f)
		...
		*/
		...
		protected:
			 ...
			 */
			void	GetNodeBoundingBox(v3f& pmin, v3f& pmax) const override { pmin.Set((float)mPosition[0], 0.0f, (float)mPosition[1]); pmax.Set((float)mPosition[0] + (float)mSize[0], (float)mDisplacement, (float)mPosition[1] + (float)mSize[1]); }
