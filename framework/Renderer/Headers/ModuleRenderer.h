...
{
	template<>
	struct hash<Kigs::Draw::PointLight>
	{
		size_t operator()(const Kigs::Draw::PointLight& pl)
		{
			size_t hash = 0;
			Kigs::Core::hash_combine(hash, pl.position, pl.attenuation, pl.specular, pl.diffuse, pl.ambient);
			return hash;
		}
	};

	template<>
	struct hash<Kigs::Draw::DirLight>
	{
		size_t operator()(const Kigs::Draw::DirLight& dl)
		{
			size_t hash = 0;
			Kigs::Core::hash_combine(hash, dl.position, dl.specular, dl.diffuse, dl.ambient);
			return hash;
		}
	};

	template<>
	struct hash<Kigs::Draw::SpotLight>
	{
		size_t operator()(const Kigs::Draw::SpotLight& sl)
		{
			size_t hash = 0;
			Kigs::Core::hash_combine(hash, sl.position, sl.directionAndCutOff, sl.attenuationAndSpotExponent, sl.diffuse, sl.specular, sl.ambient);
			return hash;
		}
	};

	template<>
	struct hash<Kigs::Draw::LightStruct>
	{
		size_t operator()(const Kigs::Draw::LightStruct& lightstruct)
		{
			size_t hash = 0;
			for (auto& l : lightstruct.pointlights)
				Kigs::Core::hash_combine(hash, l);
...
#define PREALLOCATED_VBO_COUNT					64
		...
		public:
			...
			virtual void DrawUIQuad(TravState* state, const Draw2D::UIVerticesInfo* qi) = 0;
			virtual void DrawUITriangles(TravState* state, const Draw2D::UIVerticesInfo* qi) = 0;
			...
			static Utils::FreeType_TextDrawer* mDrawer;
