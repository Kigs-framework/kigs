	...
	namespace Camera
	{
		...
		*/
		...
		public:
			...
			unsigned char* GetConvertedBuffer(int index, unsigned char* buf = NULL, const std::string& format = "");
			...
			int	GetReadyBufferIndex()
			{
				std::unique_lock<std::recursive_mutex> lk(mBuffersMutex);
				int best_found = -1;
				double best_time = -1.0f;

				for (int i = 0; i < BUFFER_COUNT; i++)
				{
					if ((mBufferState[i] == ReadyBuffer) && ((mStateTime[i] < best_time) || (best_time == -1.0f)))
					{
						best_found = i;
