...
void CoreModifiable::Upgrade(UpgradorBase* toAdd, bool doInit, bool attachMethods)
{
	...
	toAdd->UpgradeInstance(this, doInit, attachMethods);
}
...
void CoreModifiable::Downgrade(const KigsID& toRemove, bool doDestroy, bool detachMethods)
{
	...
	while (found)
	{
		if (found->getID() == toRemove)
		{
			...
			found->DowngradeInstance(this, doDestroy, detachMethods);
			break;
		}
		previous = found;
		found = (UpgradorBase*)found->getNext(LazyContentLinkedListItemStruct::ItemType::UpgradorType);
	...
	switch (destType)
	{
		...
		case ATTRIBUTE_TYPE::ARRAY:
		{
			...
			if (arraySize == 2)
			{
				...
				v2f result(ItemToEval->operator v2f());
			...
			else if (arraySize == 3)
			{
				...
				v3f result(ItemToEval->operator v3f());
				char resultBuffer[128];
