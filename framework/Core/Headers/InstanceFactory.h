...
namespace Kigs
{
	namespace Core
	{
		...
		public:
			...
			*/
			...
			public:
				...
				unordered_map<KigsID, createMethod> mClassMap;
			};

			//! get the registered module list
			const unordered_map<KigsID, ModuleAssociation>& GetModuleList() { return mModuleList; }
		...
		protected:
			...
			unordered_map<KigsID, ModuleAssociation> mModuleList;
			...
			unordered_map<KigsID, std::vector<CallbackStruct>>	mModifiableCallbackMap;
			unordered_map<KigsID, std::vector<std::string> >	mAliasList;
		};
