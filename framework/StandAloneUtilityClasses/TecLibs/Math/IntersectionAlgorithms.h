	...
	namespace Maths
	{
		...
		*/
			...
			const v3f& BBoxMin, const v3f& BBoxMax)
		{
			...
			float r = diag.squaredNorm();

			v3f Q(center);
			Q -= RayOrigin;
			float c = Q.squaredNorm();
			float v = Q.dot(RayDirection);
		...
		*/
			...
			float& SecondIntersectionDistance)
		{
			...
			float a = RayDirection.squaredNorm();
			float b = Q.dot(RayDirection);
			float c = Q.squaredNorm() - SphereRadius * SphereRadius;
			float d = b * b - a * c;
			if (d < 0.0f) return false;
			d = sqrtf(d);
		...
		*/
			...
			const v3f& BBoxMin, const v3f& BBoxMax)
		{
			...
			int sign[3];
			v3f invdir( 1.0f / RayDirection.x() , 1.0f / RayDirection.y(), 1.0f / RayDirection.z());
			sign[0] = (invdir.x() < 0);
			sign[1] = (invdir.y() < 0);
			sign[2] = (invdir.z() < 0);
		...
		*/
		...
		)
		{
			...
			TriangleCoord_u = tvec.dot(pvec)* inv_det;
			...
			TriangleCoord_v = RayDirection.dot(qvec)* inv_det;
			...
			IntersectionDistance = edge2.dot(qvec)* inv_det;

			if (IntersectionDistance >= 0.0f)
			{
				triangle_normal=edge1.cross(edge2);
