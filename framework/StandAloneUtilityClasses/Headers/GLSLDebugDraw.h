...
#include "../2DLayers/Headers/UI/UICustomDraw.h"

#include <array>

#define DEBUG_DRAW_VEC3_TYPE_DEFINED 1
using ddVec3 = Point3D;
using ddVec3Param = const Point3D;
...
#define DEBUG_DRAW_MAX_LINES 1<<20
...
namespace dd
{
	void set_rendering_size(Point2D size);
	void line2D(Point2D p1, Point2D p2, ddVec3Param color, int duration = 0);
	void rect2D(Point2D p1, Point2D p2, ddVec3Param color, int duration = 0);
...
#undef DD_PLANE_V
		...
		line(v1, v3, planeColor, durationMillis, depthEnabled);
		line(v2, v4, planeColor, durationMillis, depthEnabled);
	...
	void panel(ddVec3Param& center, ddVec3Param& planeNormal, ddVec3Param& planeColor, ddVec3Param& normalVecColor,
		const float planeScaleX, const float planeScaleY, const float normalVecScale, const int durationMillis, const bool depthEnabled)
	{
		DD_CHECK_INIT;
		...
		// A little bit of preprocessor voodoo to make things more interesting :P
#define DD_PLANE_V(v, op1, op2) \
    v.x = (center.x op1 (tangent.x * planeScaleY) op2 (bitangent.x * planeScaleX)); \
    v.y = (center.y op1 (tangent.y * planeScaleY) op2 (bitangent.y * planeScaleX)); \
    v.z = (center.z op1 (tangent.z * planeScaleY) op2 (bitangent.z * planeScaleX))
		DD_PLANE_V(v1, -, -);
		DD_PLANE_V(v2, +, -);
		DD_PLANE_V(v3, +, +);
		DD_PLANE_V(v4, -, +);
#undef DD_PLANE_V
	...
	void sphere(ddVec3Param &center, ddVec3Param &color, const int stepSize, const float radius, const int durationMillis, const bool depthEnabled)
	{
		DD_CHECK_INIT;

		int cacheSize = (int)(360 / stepSize);
		ddVec3 *cache = new ddVec3[cacheSize];
		ddVec3 radiusVec;
	...
	{
		sphere(center, color, 15, radius, durationMillis, depthEnabled);
	}
