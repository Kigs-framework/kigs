...
namespace Kigs
{
	...
	namespace Xml
	{
		...
		public:
			...
			XMLBase(const Core::SP<Core::CoreRawBuffer>& buffer);
			...
			static bool ReadFile(const std::string& filename, Core::CoreModifiable* delegateObject, const char* force_as_format);

			//! write the current xml hierarchy to a xml file
			void	WriteFile(const std::string& filename);
			Core::SP<Core::CoreRawBuffer> WriteBuffer(bool compress);
		...
		protected:
			...
			Core::SP<Core::CoreRawBuffer> mReadedRawBuffer = nullptr;
		};
		...
		public:
			...
			XMLTemplate(const Core::SP<Core::CoreRawBuffer>& buffer) : XMLBase(buffer)
			{
