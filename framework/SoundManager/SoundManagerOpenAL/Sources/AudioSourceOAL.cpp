...
IMPLEMENT_CLASS_INFO(AudioSourceOAL)


AudioSourceOAL::AudioSourceOAL(const kstl::string& name,CLASS_NAME_TREE_ARG) : AudioSource(name,PASS_CLASS_NAME_TREE_ARG)
{
...
void    AudioSourceOAL::Protected1Draw(TravState* trstate)
{
	// if no buffer, exit
	if(!myBuffer)
	{
	...
	if(myLastTime!=trstate->GetTime())
	{
		...
		else
		{
			...
			v-=myLastPos;
			v*=(kfloat)(trstate->GetTime()-myLastTime);
			myLastPos.Set(myLocalToGlobalMatrix.e[3][0],myLocalToGlobalMatrix.e[3][1],myLocalToGlobalMatrix.e[3][2]);  
			myLastTime=trstate->GetTime();
