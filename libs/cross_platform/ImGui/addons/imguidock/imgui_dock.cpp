...
#include "addons/imguidock/imgui_dock.h"
...
struct DockContext
{
	...
	Dock& getDock(const char* label, bool opened, const ImVec2& default_size)
	{
		...
		new_dock->id = id;
		new_dock->setActive();
		new_dock->status = DockStatus::Float;
	...
	void splits()
	{
		...
		for (int i = 0; i < m_docks.size(); ++i)
		{
			...
			PushID(i);
			if (!IsMouseDown(0)) dock.status = DockStatus::Docked;
			...
			if (dock.isHorizontal())
			{
				...
				if (dock.status == DockStatus::Dragged) dsize.x = io.MouseDelta.x;
			...
			else
			{
				...
				if (dock.status == DockStatus::Dragged) dsize.y = io.MouseDelta.y;
			...
			{
				dock.status = DockStatus::Dragged;
			}
	...
	Dock* getDockAt(const ImVec2& pos) const
	{
		for (int i = 0; i < m_docks.size(); ++i)
		{
			Dock& dock = *m_docks[i];
			if (dock.isContainer()) continue;
			if (dock.status != DockStatus::Docked) continue;
	...
	Dock* getRootDock()
	{
		...
		{
			if (!m_docks[i]->parent &&
				(m_docks[i]->status == DockStatus::Docked || m_docks[i]->children[0]))
			{
				return m_docks[i];
			}
		}
	...
	{
		if (dock.status == DockStatus::Float) return;
		...
		if (BeginChild(tmp, size, true))
		{
			...
			while (dock_tab)
			{
				...
				if (IsItemActive() && IsMouseDragging())
				{
					...
					doUndock(*dock_tab);
					dock_tab->status = DockStatus::Dragged;
				}
	...
	void doDock(Dock& dock, Dock* dest, Slot_ dock_slot)
	{
		...
		else
		{
			...
			container->status = DockStatus::Docked;
	...
	{
		if (dock.status == DockStatus::Docked) return;
	...
	bool begin(const char* label, bool* opened, ImGuiWindowFlags extra_flags, const ImVec2& default_size, DockStatus* status)
	{
		...
		if (opened && !*opened)
		{
			if (dock.status != DockStatus::Float)
			{
				fillLocation(dock);
				doUndock(dock);
				dock.status = DockStatus::Float;
		...
		m_current = &dock;
		if (dock.status == DockStatus::Dragged) handleDrag(dock);

		bool is_float = dock.status == DockStatus::Float;
		...
		if (!dock.active && dock.status != DockStatus::Dragged)
		{
			auto tab = &dock.getFirstTab();
			while (tab)
			{
				if (tab->active && dock.last_frame - tab->last_frame >= 2)
				{
					dock.setActive();
					doUndock(*tab);
					tab->status = DockStatus::Float;
					break;
				}
				tab = tab->next_tab;
			}
		...
		if (!dock.active && dock.status != DockStatus::Dragged) return false;
	...
	void save(const kstl::string& file)
	{
		ModuleFileManager* filemanager = (ModuleFileManager*)KigsCore::GetModule(_S_2_ID("ModuleFileManager"));
		...
		for (int i = 0; i < m_docks.size(); ++i)
		{
			...
			str << "dock" << (u64)&dock << " = {\n";
			str << "index = " << i << ",\n";
			str << "label = \"" << dock.label << "\",\n";
			str << "x = " << (int)dock.pos.x << ",\n";
			str << "y = " << (int)dock.pos.y << ",\n";
			str << "location = \"" << dock.location << "\",\n";
			str << "size_x = " << (int)dock.size.x << ",\n";
			str << "size_y = " << (int)dock.size.y << ",\n";
			str << "status = " << (int)dock.status << ",\n";
			str << "active = " << (int)dock.active << ",\n";
			str << "opened = " << (int)dock.opened << ",\n";
			str << "prev = " << (int)getDockIndex(dock.prev_tab) << ",\n";
			str << "next = " << (int)getDockIndex(dock.next_tab) << ",\n";
			str << "child0 = " << (int)getDockIndex(dock.children[0]) << ",\n";
			str << "child1 = " << (int)getDockIndex(dock.children[1]) << ",\n";
			str << "parent = " << (int)getDockIndex(dock.parent) << "\n";
			if (i < m_docks.size() - 1)
				str << "},\n";
			else
				str << "}\n";
		...
		auto output = str.str();
		filemanager->SaveFile(file.c_str(), (unsigned char*)output.c_str(), output.size());
	}
...
bool BeginDock(const char* label, bool* opened, ImGuiWindowFlags extra_flags, const ImVec2& default_size, DockStatus* status)
{
	return g_dock->begin(label, opened, extra_flags, default_size, status);
}
...
void SaveDock(const kstl::string& file)
{
	g_dock->save(file);
}
